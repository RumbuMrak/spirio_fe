...
export default function ChatEnd({ id, callId }: { id: string; callId: string }) {
  ...
  return (
    <div className="container flex flex-col items-center">
      ...
      <h1 className="mb-6 text-3xl font-bold">Chat byl ukončen</h1>
...
ChatEnd.getLayout = function getLayout(page: React.ReactElement) {
  return <Layout hideFooter>{page}</Layout>;
};
export const getServerSideProps: GetServerSideProps = async ({ query }) => {
  const id = query.id;
  const callId = query.chatid;
  if (id) {
    return {
      props: { id, callId },
    };
