...
import { Calendar, Chat, VideoCamera, PhoneCall } from '@phosphor-icons/react';
...
export default function DiscoverDetail({ data: initialData, slug }: { data: GuideProfileType; slug: string }) {
  ...
  const [selectedDate, setSelectedDate] = useState(daysRange[0]); 
      ...
      <section id="reservation" className="scroll-m-20 pb-10 sm:pb-24">
        <div className="container xl:max-w-7xl">
              ...
              {daysRange.map((date) => {
                ...
                return (
                  <button
                    ...
                    className={`w-[80px] h-[60px] flex flex-col items-center justify-center rounded-md transition-all ${
                      isAvailable
                        ? isSelected
                ...
                {calendarSlots?.some((s) => format(new Date(s.datetime), 'yyyy-MM-dd') === format(selectedDate, 'yyyy-MM-dd')) ? (
                  getHoursWithSlots(selectedDate).map((hour) => {
                    ...
                    return slots.length > 0 ? (
                      <div key={hour} className="grid-cols-4">
                        <div key={hour} className=" grid-cols-4  mb-2 flex flex-wrap gap-4 items-center">
          ...
          <div className="sm:hidden">
            ...
            {availableDays && availableDays.length > 0 && (
              <div className="mt-4 p-4 rounded-lg shadow  gap-2">
                <h5 className="text-lg font-semibold mb-2">
                  {format(selectedDate, 'd. M. EEEE', { locale: cs })}
                </h5>
                {getHoursWithSlots(selectedDate).map((hour) => {
                  ...
                  const sortedSlots = slots?.sort((a, b) => Date.parse(a.datetime) - Date.parse(b.datetime));
                  return slots.length > 0 ? (
                    <div key={hour} className="grid grid-cols-2 mb-3 pl-2 gap-2 items-center">
                      {sortedSlots.map((slot) => (
                        <ReservationButton key={slot.datetime} {...slot} id={data.id}>
                          {format(slot.datetime, 'HH:mm')} - {format(addMinutes(slot.datetime, 15), 'HH:mm')}
                        </ReservationButton>
...
const ReservationButton: React.FC<{ children: React.ReactNode; id: string } & CalendarSlotsType> = ({ children, datetime, timezone, id }) => {
  ...
  return (
    <button type="button" onClick={handleReservation} className="rounded bg-primary-725 px-2.5 py-3 text-sm font-500 transition-colors hover:bg-primary-700">
      {children}
    </button>
  );
};
