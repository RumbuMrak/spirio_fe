...
export default function PasswordInit({ token, email }: { token: string; email: string }) {
  ...
  const onSubmit = handleSubmit(async (data) => {
    console.log('Submitting password init', data.password);
    console.log('Submitting password init', token);
    await logout().catch(() => {});
    return api
      .post(
        '/password/reset',
  ...
  return (
    <>
      <div className="flex h-full grow flex-col">
        <H3 tag="h1">Vytvoření hesla</H3>
        <p className="mt-2.5">Po nastavení hesla se budeš moct kdykoli přihlásit a využívat SPIRIO naplno.</p>
...
PasswordInit.getLayout = function getLayout(page: React.ReactElement) {
  return (
    <Layout hideFooter>
